...
import { seedGames, seedItemDetails, seedItems } from "./game";
...
const d1HttpCallback: Parameters<typeof proxyDrizzle>[0] = async (
  sql,
  params,
  method
) => {
...
export type DrizzleInstance =
  | ReturnType<typeof sqliteDrizzle<typeof schema>>
  | ReturnType<typeof proxyDrizzle<typeof schema>>;

// biome-ignore lint/suspicious/useAwait: <we need to use await for the database connection>
async function createDatabaseConnection(
  ...
  return sqliteDrizzle(createClient({ url: `file:${localDbPath}` }), {
    schema,
  });
}
