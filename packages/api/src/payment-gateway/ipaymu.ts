...
import type { CreatePaymentRequest, CreatePaymentResponse } from "./abstract";
import { PaymentGateway } from "./abstract";

/**
 * iPaymu API response structure
 */
interface IpaymuApiResponse {
  Status: number;
  Message: string;
  Data?: {
    SessionId: string;
...
export class IpaymuGateway extends PaymentGateway {
  ...
  constructor(baseUrl: string, apiKey: string, va: string) {
    super();
  ...
  async createPayment(
    ...
    // 2. String to sign: "POST:{va}:{bodyHash}:{apiKey}"
    // 3. HMAC-SHA256 of string to sign with apiKey
